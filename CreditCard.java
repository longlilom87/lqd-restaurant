...
//		layer.add(panel);
		...
		txtrFafw.setFont(new Font("Monospaced", Font.PLAIN, 40));
		...
		bOK.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String t;
				if(balance >=0) t = "Your account now have "+balance;
				else t = "Now, you owe "+(-balance)+" to the banking";
	...
	private void login() throws SQLException {
		...
		for(String t: bankAccount) {
			if(usernameField.getText().equals(t)) {
				...
				PreparedStatement stmt  =c.prepareStatement("SELECT password, balance FROM Credit WHERE username = '"+t+"'");
		        ResultSet result = stmt.executeQuery();
		        
				if(passwordField.getText().equals(result.getString("password"))) {
					username = t;
					balance = result.getInt("balance");
					intro = "Your account have "+balance+"\n\nYour bill that you meal at\nLQD restaurant is "+Menu.total+"\n\nAfter paying the bill, your\naccount have "+(balance-Menu.total);
